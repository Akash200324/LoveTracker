...
for i, line in enumerate(lines):
    if '<!-- Add Memory Modal -->' in line:
        start_modal = i
    if start_modal != -1 and i > start_modal and 'id="setupModal"' in line:
        for j in range(i-1, start_modal, -1):
            if '</div>' in lines[j]:
                end_modal = j
                break
        break
...
end_js = -1
for i, line in enumerate(lines):
    if '// NEW CAMERA MODAL JS' in line:
        start_js = i - 1
        break

if start_js != -1:
    for i in range(start_js, len(lines)):
        if '</script>' in lines[i]:
            end_js = i - 1
            break

print(f"Modal: {start_modal} to {end_modal}")
print(f"JS: {start_js} to {end_js}")

if start_modal != -1 and end_modal != -1 and start_js != -1 and end_js != -1:
...
"""
    ...
    lines[start_modal:end_modal+1] = [new_modal_html + "\n"]
    
    # Recalculate JS indices because HTML length changed
    start_js = -1
    end_js = -1
    for i, line in enumerate(lines):
        if '// NEW CAMERA MODAL JS' in line:
    ...
    lines[start_js:end_js+1] = [new_js + "\n"]
    
    with open('tracker/templates/tracker/dashboard.html', 'w', encoding='utf-8') as f:
        f.writelines(lines)
    print("Successfully replaced native camera securely!")
else:
